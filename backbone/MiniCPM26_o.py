...
if __name__ == "__main__":
    ...
    response = backbone.get_completion(
        ...
        max_tokens=36864,  # 与你原流程保持一致
        model_h=448, model_w=448,
        model_max_tokens=576, model_min_tokens=64,
        prefill_chunk=12,
        temperature=0.5,
        generate_audio=False,        # 如需语音，把它改 True 并确保 pip install soundfile
        out_wav="output.wav"
    )
    ...
    if response["wav"] is not None:
        print(f"Audio saved to {os.path.abspath('output.wav')}")
